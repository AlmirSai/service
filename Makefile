...
sales:
	docker build \
		-f deployments/docker/dockerfile.sales \
		-t $(SALES_IMAGE) \
...
metrics:
	docker build \
		-f deployments/docker/dockerfile.metrics \
		-t $(METRICS_IMAGE) \
		--build-arg BUILD_REF=$(VERSION) \
		--build-arg BUILD_DATE=$(date -u +"%Y-%m-%dT%H:%M:%SZ") \
		.
...
auth:
	docker build \
		-f deployments/docker/dockerfile.auth \
		-t $(AUTH_IMAGE) \
		--build-arg BUILD_REF=$(VERSION) \
		--build-arg BUILD_DATE=$(date -u +"%Y-%m-%dT%H:%M:%SZ") \
		.
...
dev-up:
	kind create cluster \
		--image $(KIND) \
		--name $(KIND_CLUSTER) \
		--config deployments/k8s/dev/kind-config.yaml
...
dev-services-delete:
	kustomize build deployments/k8s/dev/sales | kubectl delete -f -
	kustomize build deployments/k8s/dev/grafana | kubectl delete -f -
	kustomize build deployments/k8s/dev/tempo | kubectl delete -f -
	kustomize build deployments/k8s/dev/loki | kubectl delete -f -
	kustomize build deployments/k8s/dev/promtail | kubectl delete -f -
	kustomize build deployments/k8s/dev/database | kubectl delete -f -
